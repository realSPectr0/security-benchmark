...
def create_app(config="CTFd.config.Config"):
    ...
    with app.app_context():
        ...
        if not utils.get_config("setup"):
            from CTFd.models import Admins, Pages
            from CTFd.constants.config import ChallengeVisibilityTypes, AccountVisibilityTypes, ScoreVisibilityTypes, RegistrationVisibilityTypes
            from CTFd.utils.modes import USERS_MODE

            utils.set_config("ctf_name", "CTFd")
            utils.set_config("user_mode", USERS_MODE)
            utils.set_config("ctf_theme", DEFAULT_THEME)
            utils.set_config("challenge_visibility", ChallengeVisibilityTypes.PRIVATE)
            utils.set_config("registration_visibility", RegistrationVisibilityTypes.PUBLIC)
            utils.set_config("score_visibility", ScoreVisibilityTypes.PUBLIC)
            utils.set_config("account_visibility", AccountVisibilityTypes.PUBLIC)
            utils.set_config("setup", True)

            admin = Admins(name="admin", email="admin@ctfd.io", password="notforproduction", type="admin", hidden=True)
            page = Pages(title="CTFd", route="index", content="Welcome", draft=False)
